  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <Link href={`/${slug}`} className="text-sm text-[var(--ink-soft)] hover:text-[var(--brand)]">
          ← {event.name}
        </Link>
      ...
      {summaries.length === 0 ? (
        <p className="text-[var(--ink-soft)]">Nothing handed in yet.</p>
      ) : (
        <ul className="space-y-4">
          {summaries.map(({ cycle, docs }) => (
            <li key={cycle} className="border-t border-[var(--rule)] pt-4">
              <Link
                href={`/${slug}/contributions/${cycle}`}
                className="text-lg font-medium hover:text-[var(--brand)]"
              >
                Sprint {cycle}
              </Link>
